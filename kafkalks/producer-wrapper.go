...
func (kp *KafkaProducerWrapper) processDeliveryEvent(evt kafka.Event) error {
	...
	const semLogProducerWrapperName = "name"
	var err error
	switch ev := evt.(type) {
	case *kafka.Message:
		if ev.TopicPartition.Error != nil {
			log.Error().Err(ev.TopicPartition.Error).Int64("offset", int64(ev.TopicPartition.Offset)).Int32("partition", ev.TopicPartition.Partition).Interface("topic", ev.TopicPartition.Topic).Str(semLogProducerWrapperName, kp.name).Msg(semLogContext + " delivery failed")
			err = ev.TopicPartition.Error
		} else {
			log.Trace().Int64("offset", int64(ev.TopicPartition.Offset)).Int32("partition", ev.TopicPartition.Partition).Interface("topic", ev.TopicPartition.Topic).Str(semLogProducerWrapperName, kp.name).Msg(semLogContext + " delivery ok")
		}
	}
	return err
}
